		...
		Ender_Parser_Container *c, int level)
{
	...
	else
	{
		if (c->type == ENDER_LIST)
		{
			...
			F(thiz->out, level, "{");
			_dump_container_recursive(thiz, spc, level + 1);
			F(thiz->out, level + 1, "tmp%d = ender_container_list_new(tmp%d);", level, level + 1);
			F(thiz->out, level, "}");
		}
		else
		{
		...
		Ender_Descriptor_Type type, const char *parent)
{
	...
	F(thiz->out, 0, "/* this file is automatically generated */");
	/* the shutdown */
	F(thiz->out, 0, "static void _%s_%s_shutdown(void)", thiz->ns_name, thiz->name);
	F(thiz->out, 0, "{");
	...
	F(thiz->out, 0, "}");
	/* the initialization */
	F(thiz->out, 0, "static void _%s_%s_init(void)", thiz->ns_name, thiz->name);
	F(thiz->out, 0, "{");
	F(thiz->out, 1, "Ender_Namespace *ns;");
	F(thiz->out, 1, "Ender_Descriptor *parent = NULL;");
	F(thiz->out, 1, "static Ender_Descriptor *d = NULL;");
	F(thiz->out, 1, "if (d) return;");
	F(thiz->out, 1, "ns = %s_namespace_get();", thiz->ns_name);
	...
	if (parent)
	{
		F(thiz->out, 1, "parent = ender_namespace_descriptor_find(ns, \"%s\");", parent);
		F(thiz->out, 1, "if (!parent) parent = ender_descriptor_find(\"%s\");;", parent);
		F(thiz->out, 1, "if (!parent) return;");
	}
...
{
	printf("Run: ender_generator [OPTIONS] INPUT NAME OUTPUT\n");
	printf("Where:\n");
	printf("OPTIONS can be one of the following:\n");
